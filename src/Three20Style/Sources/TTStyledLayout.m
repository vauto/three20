...
- (void)breakLine: (TTStyle*) style {
  ...
  // If there is horizontal padding, we will need to
  // horizontally align the frames on the current line.
...
- (void)layout:(TTStyledNode*)node {
  ...
  if (_lineWidth) {
    TTStyle* style = [node isKindOfClass:[TTStyledElement class]]
      ? [self findStyleForElement:(TTStyledElement*)node]
      : nil;
    [self breakLine:style];
  }
}
